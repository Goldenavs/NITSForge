...
export function CompetitorRow({ user, fadeUpVariant, onClick }: CompetitorRowProps) {
  return (
    <motion.div variants={fadeUpVariant}>
      <Card
        onClick={onClick}
        className={`cursor-pointer overflow-hidden transition-all duration-300 group ${user.isCurrentUser
        ...
        <CardContent className="p-3 sm:p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              <div className={`hidden sm:flex w-10 h-10 rounded-full border items-center justify-center font-display font-bold text-lg leading-none pt-1 overflow-hidden ${user.isCurrentUser ? 'border-primary text-primary bg-primary/10' : 'border-borderline/80 text-text-main bg-surface-2'
                }`}>
                {user.avatar_url ? (
